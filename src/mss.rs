...
pub trait Signer<T, U> {
    fn KeyGen(&self, rng: &mut impl rand::RAND) -> (SecretKey, Vec<T>);
    fn Sign(&self, sk: &SecretKey, M: &Vec<U>, rng: &mut impl rand::RAND) -> Signature<U, T>;
...
impl MercurialScheme {
    ...
    pub fn ConvertSK(&self, sk: SecretKey, rho: &big::BIG) -> SecretKey {
        // converts sk with randomness rho
        let mut new_sk = sk.0;
        for i in 0..self.ell {
            new_sk[i] = big::BIG::modmul(&new_sk[i], &rho, &self.r);
        }
        return SecretKey(new_sk);
    }
}

impl Signer<ECP2, ECP> for MercurialScheme {
    fn KeyGen(&self, rng: &mut impl rand::RAND) -> (SecretKey, Vec<ecp2::ECP2>) {
        ...
        return (SecretKey(sk), pk);
    }

    fn Sign(&self, sk: &SecretKey, M: &Vec<ecp::ECP>, rng: &mut impl rand::RAND) -> Signature<ECP, ECP2> {
        ...
        let mut Z = M[0].mul(&sk.0[0]);
        for i in 1..self.ell {
            Z.add(&(M[i].mul(&sk.0[i])));
        }
        Z = Z.mul(&y);
...
impl Signer<ECP, ECP2> for MercurialScheme {
    fn KeyGen(&self, rng: &mut impl rand::RAND) -> (SecretKey, Vec<ECP>) {
        // generates secret key sk, public key pk pair
        let mut sk = SecretKey(Vec::with_capacity(self.ell));
    ...
    fn Sign(&self, sk: &SecretKey, M: &Vec<ECP2>, rng: &mut impl rand::RAND) -> Signature<ECP2, ECP> {
        // signs a message M using secret key sk
        let y = big::BIG::randomnum(&self.r, rng);
        let mut y_inv = big::BIG::new_copy(&y);
        big::BIG::invmodp(&mut y_inv, &self.r);
